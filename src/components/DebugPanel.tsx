...
import { checkNetworkStatus, invalidateAllQueries, clearQueryCache, queryClient } from '@/config/queryClient';
...
const DebugPanel: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    const interval = setInterval(updateQueryCache, 5000);
              ...
              <div key={index} className="flex items-center justify-between p-2 border rounded">
                ...
                <div className="flex gap-2">
                  <Badge variant={query.state === 'success' ? 'default' : 
                                 query.state === 'loading' ? 'secondary' : 'destructive'}>
                    {query.state}
                  </Badge>
