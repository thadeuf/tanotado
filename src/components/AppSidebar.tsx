...
export function AppSidebar() {
  ...
  const getNavCls = ({
    ...
    isActive: boolean;
  }) => isActive ? "bg-gradient-to-r from-tanotado-pink/20 to-tanotado-purple/20 text-tanotado-navy font-medium" : "hover:bg-muted/50 text-muted-foreground hover:text-foreground";
  const handleLogout = () => {
    logout();
  };
  return <Sidebar className="border-r" collapsible="icon">
      <SidebarContent className="px-0">
        ...
        <div className="px-4 py-6 border-b flex justify-center">
          {!collapsed ? <img src="/lovable-uploads/4a78275e-0af1-4fd5-ae15-9d988197bca6.png" alt="tanotado" className="h-16 w-auto" /> : <div className="w-8 h-8 flex items-center justify-center">
        ...
        <SidebarGroup className="flex-1">
          <SidebarGroupContent>
            <SidebarMenu className="px-2">
              ...
              {/* Logout Button */}
              <SidebarMenuItem>
                <SidebarMenuButton onClick={handleLogout} className={`h-12 hover:bg-destructive/10 hover:text-destructive ${collapsed ? 'justify-center px-3' : 'px-4'}`}>
