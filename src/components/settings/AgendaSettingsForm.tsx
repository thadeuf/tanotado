...
// --- ALTERADO: Schema combinado com a nova flag ---
const agendaSettingsSchema = z.object({
  ...
  google_calendar_enabled: z.boolean().default(false), // CAMPO ADICIONADO
...
export const AgendaSettingsForm: React.FC<AgendaSettingsFormProps> = ({ onSuccess }) => {
    ...
    // --- ALTERADO: Query que busca tanto as configurações quanto o status da conexão com o Google ---
    ...
    const mutation = useMutation({
        mutationFn: async (data: AgendaSettingsFormData) => {
            ...
            const { error } = await supabase.from('user_settings').upsert({
                user_id: user.id, working_hours: working_hours as any, appointment_duration: parseInt(data.interval, 10),
                google_calendar_enabled: data.google_calendar_enabled, // Salva o estado do switch
    ...
    // --- ADICIONADO: Mutation para revogar o token do Google ---
    const revokeMutation = useMutation({
        mutationFn: () => supabase.functions.invoke('google-revoke-token'),
        onSuccess: () => {
            toast({ title: "Desconectado do Google com sucesso!", description: "A permissão foi revogada." });
            form.setValue('google_calendar_enabled', false);
            queryClient.invalidateQueries({ queryKey: ['user_settings_and_profile', user?.id] });
        },
        onError: (error: any) => toast({ title: "Erro ao desconectar", description: error.message, variant: 'destructive' }),
        onSettled: () => setIsRevokeAlertOpen(false),
    });
    
    const handleGoogleConnect = () => {
        if (!sessionToken) {
            toast({ title: 'Aguarde', description: 'Sessão de usuário não encontrada.', variant: 'default' });
            return;
        }
        ...
        const width = 600, height = 700, left = (window.innerWidth / 2) - (width / 2), top = (window.innerHeight / 2) - (height / 2);
        const popup = window.open(authUrl.toString(), 'googleAuthPopup', `toolbar=no, menubar=no, width=${width}, height=${height}, top=${top}, left=${left}`);
  ...
  return (
    <>
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3">
                                    ...
                                    <FormControl>
                                        {/* --- ALTERADO: Lógica do Switch para disparar a desconexão --- */}
                                        <Switch
                                            checked={field.value}
        ...
        {/* --- INÍCIO DA ALTERAÇÃO: Adicionar o AlertDialog com Loading State --- */}
      <AlertDialog open={isRevokeAlertOpen} onOpenChange={setIsRevokeAlertOpen}>
          <AlertDialogContent>
              <AlertDialogHeader>
                  <AlertDialogTitle>Você tem certeza?</AlertDialogTitle>
                  <AlertDialogDescription>
                      Isso irá desconectar sua conta do Google e remover a permissão do tanotado.
                      Você precisará se conectar novamente se quiser reativar a sincronização.
                      Nenhum evento já criado será apagado da sua Google Agenda.
                  </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                  ...
                  >
                      {/* Lógica para mostrar o loader e alterar o texto durante a ação */}
                      {revokeMutation.isPending ? (
                          <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                      ) : (
                          <Trash2 className="mr-2 h-4 w-4" />
                      )}
                      {revokeMutation.isPending ? "Desconectando..." : "Sim, desconectar"}
                  </AlertDialogAction>
              </AlertDialogFooter>
          </AlertDialogContent>
      </AlertDialog>
